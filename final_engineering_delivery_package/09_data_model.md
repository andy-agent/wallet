...
| admin_users | 后台 | 后台管理员账号 | username,password_hash,role,status | PK(id), UQ(username) | role,status | 后台登录 | RBAC 以固定角色枚举实现；Admin 不单独持久化 refresh session |
| audit_logs | 审计 | 后台与关键系统动作审计 | request_id,actor_type,actor_id,module,action,target_type,target_id,before_json,after_json | PK(id) | request_id,module+action,target,created_at | 后台审计日志 | 资金、状态、配置类操作必写；通过 requestId 支持后台回查 |
...
| vpn_nodes | VPN | 节点定义 | region_id,node_code,host,port,protocol,transport_protocol,security_type,reality_public_key,server_name,short_id,flow,weight,status,health_status,last_heartbeat_at | PK(id), UQ(node_code) | region_id,status,health_status | 后台节点 | 客户端不直接面对节点，仅面对区域 |
...
| chain_configs | 支付/钱包 | 链 RPC 与策略配置 | network_code,display_name,public_rpc_url,proxy_rpc_url,direct_broadcast_enabled,proxy_broadcast_enabled,required_confirmations,explorer_tx_url_template,is_active | PK(id), UQ(network_code) | is_active | 钱包 precheck、支付扫描 | 确认数与广播兜底策略来自此表 |
| asset_catalog | 支付/钱包 | 支持资产目录 | network_code,asset_code,display_name,symbol,decimals,is_native,contract_address,wallet_visible,order_payable,is_active | PK(id), UQ(network_code,asset_code,contract_address) | network_code,is_active | 钱包首页、收银台、后台配置 | 首期只开放 SOL/TRX/USDT |
| payment_addresses | 支付 | 平台收款地址池 | network_code,asset_code,address,strategy,is_active,label | PK(id), UQ(network_code,asset_code,address) | network_code,asset_code,is_active | 订单支付目标 | 当前采用共享地址 + 唯一金额 delta |
