...
fun LegalDetailScreen(
    ...
    val doc = viewModel.document(docId) ?: return
    AppScaffold(title = "法务详情", onBack = onBack) { padding ->
        LazyColumn(modifier = Modifier.fillMaxSize().padding(padding).padding(18.dp), verticalArrangement = Arrangement.spacedBy(16.dp)) {
            item { GradientCard(title = doc.title, subtitle = doc.summary) { Text(doc.content) } }
        }
    }
}
