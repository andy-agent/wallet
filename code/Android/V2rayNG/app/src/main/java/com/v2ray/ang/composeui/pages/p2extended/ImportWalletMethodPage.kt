...
fun ImportWalletMethodRoute(
    ...
    ImportWalletMethodScreen(
        ...
        onEvent = { event ->
            ...
            when (event) {
                ImportWalletMethodEvent.PrimaryActionClicked -> if (!uiState.isLoading) onPrimaryAction()
                ImportWalletMethodEvent.SecondaryActionClicked -> if (!uiState.isLoading) onSecondaryAction?.invoke()
                else -> Unit
            }
        },
