...
data class ImportWalletMethodUiState(
    ...
    FeatureBullet(title = "ViewModel Stub", detail = "导入钱包方式 已预留事件分发与 refresh 占位。"),
    FeatureBullet(title = "Mock Repository", detail = "可通过 ImportWalletMethodPreviewState / Repository 种子替换真实接口。"),
    ...
    FeatureBullet(title = "Navigation Args", detail = "当前页面无必填参数，但已纳入统一 RouteSpec 台账。"),
        ...
        val note: String = "导入钱包方式 已按 P2 扩展页 页面补齐，可继续替换为真实业务逻辑与接口数据。",
    )
