...
let InternalAuthGuard = class InternalAuthGuard {
    ...
    canActivate(context) {
        ...
        const expectedToken = this.configService.get('INTERNAL_AUTH_TOKEN');
        if (!expectedToken) {
            throw new common_1.UnauthorizedException('Internal auth token not configured');
        }
        const token = typeof authHeader === 'string' && authHeader.startsWith('Bearer ')
            ? authHeader.slice('Bearer '.length)
            : '';
        if (token !== expectedToken) {
            throw new common_1.UnauthorizedException('Invalid internal auth token');
        }
        return true;
    }
};
